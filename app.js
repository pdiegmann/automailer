...
app.get('/dataset/:datasetid/mail/lists', function(req, res, next) {
	var start = process.hrtime();
	...
	MailingListModel.count({ dataset: datasetid }, function(err, count) {
		console.log("count:" + count);
		MailingListModel.find({ dataset: datasetid }, { __v: 0 }).sort({ created: -1 }).populate("sendTo", "-__v").populate("sentMails", "-__v").populate("answers", "-__v").limit(take).skip(skip).exec(function(err, docs) {
			if (err) {
				logger.error(err);
				return res.send(500);
			}
			...
			async.each(docs, function(doc, callback) {
				...
				async.each(doc.sendTo, function(person, callbackDeep) {
					...
					CompanyModel.findOne({ "_id": person.company }, { "__v": 0, "raw": 0 }, function(err, company) {
						if (err) {
							console.error(err);
							callbackDeep();
							return;
					...
					msg.on('body', function(stream, info) {
						...
						mailparser.on("end", function(mail) {
							...
							*/
							...
							MailModel.findOne({ "dataset": datasetid, "_id": mailIdCandidate }, function (err, originalMail) {
								if (err) {
									console.error(err);
								}
								...
								MailModel.findOne({ "dataset": datasetid, "externalId": "" + (msg.attributes && msg.attributes.uid ? msg.attributes.uid : seqno) }, function (err, oldResponse) {
									...
									response.save(function(err) {
										...
										MailingListModel.findOne({ "dataset": datasetid, "_id": maillistIdCandidate }, function(err, mailingList) {
											if (err) {
												console.error(err);
											}
										...
										if (originalMail) {
											...
											PersonModel.findOne({ "dataset": datasetid, "_id": originalMail.person }, function(err, person) {
												if (err) {
													console.error(err);
												}
											...
											PersonModel.find({ "dataset": datasetid, "mailAddresses.address": mail.from[0].address }, function(err, persons) {
												if (err) {
													console.error(err);
												}
...
function processResponse(response, originalMail, mailingList) {
